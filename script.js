...
const butaoPulo = document.querySelector('.butao_pulo');
const butaoAtaque = document.querySelector('.butao_ataque');
let jumpContagem = 0;
...
const ataque = () => {
    if (jogoAcabou == false && !alucard.classList.contains('jump') && !alucardAtacando) {
        ...
        alucard.style.width = '320px';
        alucard.style.height = '280px';
        alucard.style.bottom = '-25px'
        setTimeout(() => {
            if (jogoAcabou == false) {
                alucard.src = 'assets/imgs/Alucard.gif';
                alucard.style.width = '';
                alucard.style.height = '';
...
const loop = setInterval(() => {
    ...
    if (zumbiPosition <= 100 && zumbiPosition > 0 && alucardPosition < 60) {
...
const restart = () => {
    ...
    alucard.src = 'assets/imgs/Alucard.gif';
    alucard.classList.remove('dead')
    alucard.style.bottom = '0px';
    alucard.style.marginLeft = '';
...
butaoPulo.addEventListener('touchstart', jump);
butaoAtaque.addEventListener('touchstart', ataque);
